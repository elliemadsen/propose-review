...
I am proposing a research project that explores the novel application of foundation models to remote sensing. My methods include experimental data analysis, theoretical and technical writing, critical cartography, elemental media studies, topology, machine learning, and speculation. I will conduct a range of experiments using these methods to explore and explain various properties of Earth embedding data. The resulting content will be presented in an atlas in both physical and digital forms.

Over the last few years, there has been a steady release of new remote sensing foundation models in Earth Observation. RSFMs are large-scale machine learning models trained primarily on satellite imagery, but also on aerial, radar, thermal, climate, and other data. They learn—or predict—generalized spatial–temporal representations of Earth’s surface. These general-purpose models are intended to perform a range of downstream tasks, including clustering, crop type mapping, classification, and object detection via similarity search.

RSFMs compress huge amounts of Earth observation data into a unified digital representation, or *embedding*. The data is embedded as high-dimensional vectors, which together construct latent space: a raster image painted by the brushstrokes of a billion embeddings. For example, in Google’s AlphaEarth satellite embedding dataset, released in July 2025, each pixel represents 10 m, and each 10 m region is embedded into a 64-dimensional vector.

Planetary geographical data is inherently spatial-but when it is embedded, the remote sensing data constructs a new space: latent space. Imagine a 3D sphere, but make it 64 dimensions. Considering the spatial limits of human perception, how might we make sense of high-dimensional data?
...
Yet I am less interested in what geospatial embedding data can tell us about the world than in what this methodology tells us about ourselves. Applying theories of the archive allows us to think of big data in terms of narrative, knowledge, and control. What if, instead of taking these data at face value and using embeddings as a method to understand the Earth, the subject of study is the embeddings themselves—as a form of matter in their own right?
...
I will apply computational methods in machine learning, GIS, and exploratory data analysis to uncover and illustrate the properties, limits, edge cases, opportunities, and formal characteristics associated with Earth embedding data.
...
* **Speculative**: I will question how humans can sense and reason about immaterial space.
...
* [Review Poster](https://github.com/elliemadsen/satellite-embeddings/blob/main/poster/poster.pdf)
...
Thus far, I have been working with Google DeepMind’s AlphaEarth Satellite Embedding dataset, which was released just in July 2025 and is accessible through Google Earth Engine. This model encodes global spatial-temporal conditions into 64-dimensional embeddings at the resolution of 10m per pixel. The temporal resolution is annual, with one set of embeddings per year available from the years 2017-2024.

The fact that the global embedding data is precomputed and hosted within Google Earth Engine makes it far more accessible than alternatives such as TESSERA, which would require substantial compute resources to download and host independently. Offline analysis with AlphaEarth data can be easily conducted using Python scripts. However, Google Earth Engine’s API does not allow the embedding data to be queried directly in a live web application, and the dataset’s massive petabyte scale prevents local downloads at any meaningful resolution. Additionally, raw data redistribution is restricted for proprietary reasons. There are ways to work within these constraings-for example, I converted global raw embedding data into false-color GeoTIFFs and generated low-resolution PNG tiles, enabling the creation of a web-based globe hosted independently of Google Earth Engine [here](https://elliemadsen.github.io/satellite-embeddings/web-maps/globe.html).

In the Spring, I will explore the use of NASA’s open-source Prithvi 2.0 model as a research assistant for Columbia’s Center for Climate Systems Research, in collaboration with NASA’s Goddard Institute for Space Studies. This work focuses on modeling climate impacts on the New York watershed and will grant me access to NASA compute resources and datasets.

While my final output will be an atlas of general embedding earth technology, not specifically Google’s AlphaEarth model, it is likely that I will use this model for all data analysis and visualizations due to technical constraints.
